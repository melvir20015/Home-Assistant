...
- **Contrato efectivo del ciclo:** el contrato base se calcula con `sp_cool_target`, `off_cool`, `on_cool`, columna, franja, clima y presión de confort nocturna. Si ese contexto cambia mientras el ciclo sigue activo, Night resincroniza SP/fan contra el nuevo contrato, pero conserva `input_datetime.ac_night_cool_cycle_started_ts`; por lo tanto, no reinicia el contador de 30 minutos del ciclo activo.
- **Refuerzo progresivo operativo:** cada ciclo de enfriamiento inicia con `sp_cool_target` y `input_number.ac_night_cool_reinforcement_count=0`. Si pasan 30 minutos y `Tin > off_cool` con `hvac_mode=cool` y presencia válida, Night aplica `sp_cool_target - 1`. A los 60 minutos aplica `sp_cool_target - 2` y a los 90 minutos `sp_cool_target - 3`, siempre que todavía no se haya llegado a `off_cool`.
...
- **Observabilidad:** los logs de resincronización contextual y refuerzo agresivo exponen `trace_id`, `sp_contractual`, `sp_cool_normal`, `sp_cool_aggressive`, `sp_solicitado`, `sp_reportado`, `delta_reportado`, motivo y contexto para auditar si el contrato o la lectura del equipo explica cada ajuste.
